...
# 2. DATA ENGINE (UPDATED TO CLEAN REPEATED HEADERS)
...
def load_data():
    try:
        ...
        # --- CLEANER START ---
        # 1. Remove rows that are just repeated headers (common in manual entry sheets)
        if 'Player/Team' in df.columns:
            df = df[df['Player/Team'] != 'Player/Team']
            
        # 2. Remove rows where Player/Team is empty/NaN
...
if isinstance(full_df, str): st.error(f"⚠️ DATA ERROR: {full_df}")
    ...
    with tabs[1]:
        if not t_stats.empty:
            ...
            t_disp = t_stats.sort_values('Total_Win', ascending=False)[['Record', 'PTS/G', 'AST/G', 'REB/G', 'FG%', 'DefRtg', 'OffRtg']]
    ...
    with tabs[2]:
        ...
        with c_p:
            if not p_stats.empty:
                t10p = p_stats.nlargest(10, l_cat)[[l_cat]]
    ...
    with tabs[3]:
        ...
        if not source.empty:
            ...
            for label, col in [('PPG', 'PTS/G'), ('APG', 'AST/G'), ('RPG', 'REB/G'), ('PIE', 'PIE'), ('FG%', 'FG%')]:
                c1, cm, c2 = st.columns([2, 1, 2])
                c1.metric(p1, d1[col], round(d1[col]-d2[col], 2))
                cm.markdown(f"<div style='text-align:center;'>{label}</div>", unsafe_allow_html=True)
                c2.metric(p2, d2[col], round(d2[col]-d1[col], 2))

    with tabs[4]:
        st.header("🏆 POSTSEASON")
        mode = st.radio("Tournament Type", ["Playoffs (9k)", "Tournament (8k)"], horizontal=True)
        ...
        if post_df.empty: st.info(f"No {mode} data found.")
            ...
            st.dataframe(ps_stats[['GP', 'PTS/G', 'AST/G', 'REB/G', 'FG%', 'PIE']].sort_values('PIE', ascending=False), width="stretch")
    ...
    with tabs[6]:
        ...
        if st.text_input("Passcode", type="password") == "BPL2026":
            ...
            st.dataframe(p_stats[['Total_PTS', 'Total_REB', 'Total_AST', 'Total_Win', 'Total_DD']].sort_values('Total_PTS', ascending=False), width="stretch")
